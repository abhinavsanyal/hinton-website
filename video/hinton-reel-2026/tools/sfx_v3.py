...
v3 (90 s): a leaner pass — real crowd/street sound now comes from the Kling clips (mixed in tools/mix_v3.py)."""
...
# ---------- timeline (v3, 90 s) ----------
proj = projector(12.2)
proj_env = np.concatenate([np.ones(int(1.2 * SR)), np.linspace(1, 0.4, int(1.3 * SR)), np.full(int(9.2 * SR), 0.4), np.linspace(0.4, 0, int(0.5 * SR))])
place(proj[: len(proj_env)] * proj_env[: len(proj)], 0.0, 0.22)
for tb in [0.36, 0.72]:
    place(beep(1000, 0.045), tb, 0.16)
for tw in [3.12, 6.6, 9.45, 11.9, 16.75, 19.8, 23.3, 29.85, 32.3, 34.72, 42.2, 53.35, 58.5, 63.7, 67.5, 74.45]:
    place(whoosh(0.55, 200, 4500), tw - 0.25, 0.26)
for tb, g in [(1.1, 0.7), (46.43, 0.35), (49.88, 0.55), (80.5, 0.45)]:
    place(boom(), tb, g)
for tb in [31.45, 39.75, 40.1, 40.45, 40.8]:
    place(thud(), tb, 0.45); place(paper(0.2), tb, 0.16)
for tb in [3.35, 9.75, 12.15]:
    place(paper(0.3), tb, 0.2)
place(paper(), 4.95, 0.22, 0.4)
for i in range(3):
    place(paper(0.35), 38.9 + i * 0.12, 0.18)
for i in range(6):
    place(paper(0.3), 36.7 + i * 0.33, 0.16, (-1) ** i * 0.4)
for tp, f in [(2.55, 820), (4.25, 900), (5.5, 760), (8.1, 900), (10.25, 900), (10.4, 1080), (10.85, 520), (12.4, 880), (15.55, 600), (18.9, 820), (22.3, 700), (30.15, 900), (54.97, 900), (56.25, 1150), (58.8, 900), (64.0, 760), (68.8, 900)]:
    place(pop(f), tp, 0.14)
for i in range(8):
    place(pop(600 * 2 ** (i / 7)), 13.45 + i * 0.26, 0.16, (i % 2) * 0.6 - 0.3)
for i in range(16):
    place(paper(0.2), 23.55 + i * 0.395, 0.12, (-1) ** i * 0.3)
for i in range(14):
    place(typekey(), 1.7 + i * (0.8 / 14), 0.14)
for i in range(36):
    place(typekey() * 0.8, 53.85 + i * (1.9 / 36) + rng.uniform(-0.01, 0.01), 0.1)
place(tick(0.05, 1500) * 3, 55.85, 0.22)
place(shimmer(1.2), 55.97, 0.18)
for i in range(30):
    place(tick(0.02, 3000), 30.3 + i * 0.03, 0.05)
for i in range(60):
    place(tick(0.02, 6000), 44.95 + i * 0.0125, 0.07, rng.uniform(-0.8, 0.8))
place(riser(1.7), 47.95, 0.32)
for i in range(4):
    place(pop(700 + i * 150, 0.1), 51.8 + i * 0.36, 0.16)
for i in range(9):
