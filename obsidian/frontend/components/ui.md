...
| `FlareMedia` | **Client** | Media tile with a cinematic **anamorphic lens flare** that tracks the cursor: pointer position drives `rx`/`ry` tilt plus three `mix-blend-screen` layers (wide horizontal streak, cool radial core at the cursor, warm ghost mirrored through the centre). Video is `preload="none"` and plays only on hover; `GlassPlayButton` mounts on hover. With no `src` it renders a dashed placeholder, so a service page can ship before its footage exists. Used by the [[service-pages]] media walls. |
