...
**Context.** The site needed a dedicated page per capability (AI TVC production,
brand & product films, vertical micro dramas, AI storyboards & moodboards, 2D &
3D animation) to rank on one-intent-one-URL searches, reachable from a new nav
menu. Hard constraint from the brief: **additive only** — the Showreel landing
page and its timeline must not be touched. The brief also asked for "GSAP scroll
animations with a 3D camera", which conflicts with ADR-0002.

**Decision.**

1. **URL shape `/services/<slug>`, not root-level `/<slug>`.** A root-level
   `[slug]` segment would swallow every unmatched path in the app. The `/services`
   hub also doubles as the internal-link distribution node the SEO plan wants.
2. **`dynamicParams = false`.** The service set is fixed and enumerated by
   `generateStaticParams`, so off-list slugs hard-404 instead of rendering an
   on-demand soft-404 (200 status + "not found" body) that a crawler would index.
3. **No GSAP.** GSAP is not installed and ADR-0002 bans non-spring motion. The
   requested "3D camera in 3D space" is reproduced with the Showreel's own
   technique: one `ProgressTrigger` scrubbing a single react-spring value into a
   CSS `perspective` + `translateZ` rig. Visually equivalent, architecturally
   consistent, zero new dependencies.
4. **One content module drives everything.** `data/mocks/services.ts` feeds the
   nav dropdown, the hub grid, `generateStaticParams`, the sitemap, the page copy
   and the JSON-LD. There is no second list to keep in sync.
5. **`SiteHeader` made page-agnostic additively.** New `awaitLoader?: boolean`
   (default `true`, so home is unchanged) decouples the reveal from the home-only
   `IntroLoader`; hash hrefs are prefixed with `/` off-home. The header then
   mounts unchanged on `/services`, `/services/*` and `/work`.
6. **FAQ answers stay visible.** No accordion. Collapsed text is unreliably
   extracted by AI answer engines and reads as cloaking.

**Consequences.** Five pages prerender as static HTML (good LCP, fully
crawlable). Adding a service is a data-only change. The Showreel timeline,
`views/home.tsx`, and the animation engine were not modified. The cost of
declining GSAP is that any future scroll choreography must also be expressed as
spring interpolations — consistent with the rest of the codebase. Video is
`preload="none"` and only plays on hover, so the media walls do not compete with
the LCP element. See [[service-pages]].
