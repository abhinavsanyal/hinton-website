...
- **Six new pages** — a `/services` hub plus five capability pages
  (`ai-tvc-production`, `brand-and-product-films`, `vertical-micro-dramas`,
  `ai-storyboards-and-moodboards`, `2d-and-3d-animation`), built as a
  hub-and-spoke SEO cluster. All five prerender as static HTML (SSG,
  `dynamicParams = false`). Purely additive — the Showreel landing page, its
  timeline, and the animation engine were not touched. See [[service-pages]] and
  [[decisions-log]] ADR-0025.
- **New `data/mocks/services.ts`** — the single content source. It drives the nav
  dropdown, the hub grid, `generateStaticParams`, the sitemap, every page section
  and the JSON-LD. Adding a service is a data-only change.
- **New `views/services/`** — `ServiceView` (shared layout) + `ServiceHero`,
  `ServiceNarrative`, `ServiceMediaWall`, `ServiceProcess`, `ServiceFaqSection`,
  `ServiceCta`, and `ServicesIndexView`. The hero reuses the Showreel's camera
  technique — one `ProgressTrigger` scrubbing a single spring into a CSS
  `perspective` + `translateZ` rig — **not** GSAP, which is banned by ADR-0002 and
  not installed.
- **New `components/ui/flare-media.tsx`** — media tile with a cursor-tracking
  anamorphic lens flare (three `mix-blend-screen` layers) and spring tilt. Video
  is `preload="none"`, plays on hover only; a missing `src` renders a dashed
  placeholder so pages can ship ahead of their footage.
- **New `views/home/services-menu.tsx`** — the glass dropdown in the nav.
  `NavLink` gained an optional `menu?: NavMenuItem[]`; when present `SiteHeader`
  renders the dropdown instead of a plain link, and the mobile sheet nests the
  sub-links. Existing nav entries are unaffected.
- **`SiteHeader` is now page-agnostic** — new `awaitLoader?: boolean` prop
  (default `true`, home unchanged) decouples the reveal from the home-only
  `IntroLoader`, and `#section` hrefs resolve to `/#section` off-home. **`/work`
  now has the navbar** (it previously had none); its top padding went
  `20vmin → 24vmin` and the floating close button drops below the bar on phones.
- **`getServiceStructuredData()`** added to `utils/seo/structured-data.ts` —
  `Service` + `OfferCatalog` + `FAQPage` + `HowTo` + `BreadcrumbList`, tied by
  `@id` to the existing Organization node. **Sitemap** now lists `/services`,
  `/work` and all five service routes.
