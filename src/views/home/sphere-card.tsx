  ...
  return (
    <div className="size-full">
      {/* Carousel-face: violet mesh gradient + product chrome (top label + url,
          ...
          IMPORTANT: This face has its own opacity (cardFaceOpacity) that fades it
          to 0 EARLY — as the H mask starts growing — so the maroon background
          completely dissolves rather than lingering behind the mask edges. */}
      <animated.div
        aria-hidden="true"
        className="absolute inset-0 overflow-hidden rounded-card"
        style={{
          background: "var(--card-violet)",
          opacity: p.to(cardFaceOpacity),
        }}
      >
      ...
      <animated.div
        ...
        style={{
          ...maskStyle,
          WebkitMaskSize: p.to((v) => starMaskSize(v, vmin, geo)),
          maskSize: p.to((v) => starMaskSize(v, vmin, geo)),
      ...
      >
        ...
        >
          ...
          {/* Metallic H logo — polished chrome with sharp horizon reflections */}
          <animated.span
            aria-hidden="true"
            className="absolute left-1/2 top-1/2 z-[3] h-[22vmin] w-[22vmin]"
            style={{
              ...maskStyle,
              ...
              /* Polished chrome base gradient with sharp horizon contrast */
              background: "linear-gradient(135deg, #f0f2f5 0%, #e0e4eb 25%, #6a7585 46%, #1a2230 50%, #768192 54%, #d1d6e0 75%, #ffffff 100%)",
            }}
          >
            {/* Intense diagonal shimmer sweep overlay */}
            <span
              className="absolute inset-0 pointer-events-none"
              style={{
                background: "linear-gradient(110deg, transparent 0%, rgba(255,255,255,0) 40%, rgba(255,255,255,0.95) 50%, rgba(255,255,255,0) 60%, transparent 100%)",
                backgroundSize: "250% 100%",
                animation: "chromeShimmer 4s cubic-bezier(0.4, 0, 0.2, 1) infinite",
              }}
            />
          </animated.span>
