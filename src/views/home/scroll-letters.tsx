...
export interface ScrollLettersProps {
  ...
  letterClassName?: (char: string, index: number) => string;
  renderAddon?: (char: string, index: number) => ReactNode;
}
      ...
      {chars.map((ch, i) => {
        ...
        return (
          ...
          <span key={i} className="inline-flex align-bottom">
            <animated.span
              className={`${className || ""} ${letterClassName ? letterClassName(ch, idx) : ""} relative`}
              style={{
            ...
            >
              {ch}
              {renderAddon && renderAddon(ch, idx)}
            </animated.span>
          </span>
        );
      })}
