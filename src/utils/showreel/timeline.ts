...
export const card4Opacity = (p: number) => {
  // Fade the whole card-4 wrapper (sphere scene + face) LATE — once the
  // portfolio has entered and fully covered the sphere. The face itself fades
  // earlier via the separate cardFaceOpacity, so this only matters for the
  // sphere/particles/headings.
  const v = vScroll(p);
  const sphereFade = clamp01((v - (PS + 600)) / 400);
  return 1 - sphereFade;
};

/**
 * Separate fade for the maroon/violet card-4 FACE (the carousel-slot background).
 * Stays fully visible while the H mask is still growing over the card, then
 * dissolves once the mask has expanded past the card edges. This creates an
 * organic, grand feel — the card face is covered by the growing black mask,
 * not abruptly hidden.
 */
export const cardFaceOpacity = (p: number) => {
  // The face stays at full opacity until phase4 = 0.65 (mask is very large),
  // then fades smoothly to 0 by phase4 = 0.95 (just before the portfolio).
  // This ensures the maroon card stays visible until we have fully transitioned
  // through the brand logo mask.
  const t = clamp01((phase4(p) - 0.65) / 0.30);
  return 1 - smooth(t);
};
...
export const starMaskSize = (p: number, vmin: number, geo: ShowreelGeo = DESKTOP_GEO) => {
  const t = phase4(p);
  // Match the metallic logo's optical scale at the start so the mask cutout perfectly aligns
  const baseSize = 22 * sphereSceneScale(p, vmin, geo);
  // Smoothstep-then-accelerate: gentle start, dramatic finish
  const eased = t < 0.4 ? smooth(t / 0.4) * 0.15 : 0.15 + Math.pow((t - 0.4) / 0.6, 2.5) * 0.85;
  return `${baseSize + eased * 1600}vmin`;
};
...
export const sphereLogoTransform = (p: number) => {
  // Keep the metallic logo perfectly straight and aligned to its 22vmin size.
  // The black mask hole starts at 22vmin, so this ensures they match exactly
  // and the logo doesn't tilt or warp at the end of the animation.
  return `translate(-50%, -50%)`;
};
export const sphereLogoOpacity = (p: number) => {
...
export interface GridItem {
  ...
  z: number; // px depth
  scale: number;
  image: string; // legacy — kept for type compat, now empty string
  /** Video URL for the grid tile (one of the 6 portfolio clips). */
  video: string;
}
...
export const gridItemTransform = (item: GridItem) => {
  const txStr = item.tx < 0 ? `- ${Math.abs(item.tx)}` : `+ ${item.tx}`;
  const tyStr = item.ty < 0 ? `- ${Math.abs(item.ty)}` : `+ ${item.ty}`;
  return `translate(calc(-50% ${txStr}vw), calc(-50% ${tyStr}vh)) translateZ(${item.z}px) scale(${item.scale})`;
};
