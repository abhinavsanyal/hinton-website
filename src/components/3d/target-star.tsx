...
/**
 * Procedural "H" — traced from the actual Hinton Studios brand mark.
 * The H has thick pillars with angular chevron notches on the inner edges
 * where they meet the cross-bar, creating the logo's distinctive geometric
 * faceted look. Built from THREE.Shape + ExtrudeGeometry.
 */
function createHShape(): THREE.BufferGeometry {
  const shape = new THREE.Shape();
  // Logo proportions — the H is roughly square with thick pillars
  const W = 2.6;         // total width
  const H = 3.0;         // total height
  const pW = 0.65;       // pillar width
  const bH = 0.55;       // cross-bar height
  const bY = (H - bH) / 2; // bar bottom Y
  const bT = bY + bH;     // bar top Y
  const chev = 0.25;      // chevron notch depth (how far in from pillar inner edge)

  // Draw the H outline with chevron notches. Starting bottom-left, clockwise.
  // Left pillar bottom
  shape.moveTo(-W / 2, -H / 2);
  // Left pillar outer-left edge → top
  shape.lineTo(-W / 2, H / 2);
  // Left pillar top → inner edge
  shape.lineTo(-W / 2 + pW, H / 2);
  // Left pillar inner edge down to top chevron point
  shape.lineTo(-W / 2 + pW, bT + chev);
  // Chevron notch inward (the angular cut) → bar top
  shape.lineTo(-W / 2 + pW - chev, bT);
  // Straight across to right pillar chevron
  shape.lineTo(W / 2 - pW + chev, bT);
  // Right pillar top chevron point
  shape.lineTo(W / 2 - pW, bT + chev);
  // Right pillar inner edge → top
  shape.lineTo(W / 2 - pW, H / 2);
  // Right pillar top outer
  shape.lineTo(W / 2, H / 2);
  // Right pillar outer-right edge → bottom
  shape.lineTo(W / 2, -H / 2);
  // Right pillar bottom → inner edge
  shape.lineTo(W / 2 - pW, -H / 2);
  // Right pillar inner edge up to bottom chevron point
  shape.lineTo(W / 2 - pW, bY - chev);
  // Chevron notch inward → bar bottom
  shape.lineTo(W / 2 - pW + chev, bY);
  // Straight across to left pillar chevron
  shape.lineTo(-W / 2 + pW - chev, bY);
  // Left pillar bottom chevron point
  shape.lineTo(-W / 2 + pW, bY - chev);
  // Left pillar inner edge → bottom
  shape.lineTo(-W / 2 + pW, -H / 2);
  // Close
  shape.lineTo(-W / 2, -H / 2);
...
function HModel() {
  ...
  // Chrome "H" mesh — same material as the original star model.
  const meshData = useMemo(() => {
    const geo = createHShape();
...
const BG_FRAG = /* glsl */ `
  ...
  void main(){
    ...
    /* Hinton Studios brand palette — deep crimson / cherry / warm glow. */
    vec3 night    = vec3(0.045, 0.015, 0.020);
    vec3 indigo   = vec3(0.32,  0.06,  0.08);
    vec3 violet   = vec3(0.72,  0.14,  0.18);
    vec3 electric = vec3(0.92,  0.24,  0.22);
    vec3 glow     = vec3(0.99,  0.72,  0.72);
    ...
    col += vec3(1.0, 0.68, 0.68) * hotGlow * 0.7;
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        {/* Key light — dramatic from above */}
        <directionalLight color={0xffffff} intensity={2.5} position={[2.5, 5.0, 4.0]} />
        {/* Fill light — cool blue from below-left */}
        <directionalLight color={0xcfe0ff} intensity={0.7} position={[-3.0, -1.5, 2.0]} />
        {/* Spotlight from far above — cinematic top-down pool */}
        <spotLight
          color={0xffffff}
          intensity={8.0}
          position={[0, 12, 6]}
          ...
          castShadow={false}
        />
        {/* Warm backlight from far behind — rim lighting effect */}
        <spotLight
          color={0xffccaa}
          intensity={4.0}
          position={[-4, 8, -10]}
        ...
        <Suspense fallback={null}>
          ...
          <EffectComposer enableNormalPass={false} multisampling={4}>
            <DepthOfField target={[1.46, -0.43, -0.33]} focalLength={0.04} bokehScale={10} height={700} />
